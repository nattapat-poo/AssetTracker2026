...
async function runTests() {
  ...
  if (payload.config.APP_VERSION !== 'v1.1.8l' && payload.config.APP_VERSION !== 'v1.1.8k' && payload.config.APP_VERSION !== 'v1.1.8j' && payload.config.APP_VERSION !== 'v1.1.8i' && payload.config.APP_VERSION !== 'v1.1.8h' && payload.config.APP_VERSION !== 'v1.1.8g') throw new Error(`Expected APP_VERSION to be v1.1.8l, got ${payload.config.APP_VERSION}`);
  ...
  if (!configSyncRes.success || (configSyncRes.appVersion !== 'v1.1.8l' && configSyncRes.appVersion !== 'v1.1.8k' && configSyncRes.appVersion !== 'v1.1.8j' && configSyncRes.appVersion !== 'v1.1.8i' && configSyncRes.appVersion !== 'v1.1.8h' && configSyncRes.appVersion !== 'v1.1.8g' && configSyncRes.appVersion !== 'v1.1.8f' && configSyncRes.appVersion !== 'v1.1.8e' && configSyncRes.appVersion !== 'v1.1.8d' && configSyncRes.appVersion !== 'v1.1.8b')) {
  ...
  // Verify APP_CONFIG.VERSION is v1.1.8l, v1.1.8k, v1.1.8j, v1.1.8i, v1.1.8h, v1.1.8g, v1.1.8f, v1.1.8e, or v1.1.8d
  if (!configJs.includes('VERSION: "v1.1.8l"') && !configJs.includes('VERSION: "v1.1.8k"') && !configJs.includes('VERSION: "v1.1.8j"') && !configJs.includes('VERSION: "v1.1.8i"') && !configJs.includes('VERSION: "v1.1.8h"') && !configJs.includes('VERSION: "v1.1.8g"') && !configJs.includes('VERSION: "v1.1.8f"') && !configJs.includes('VERSION: "v1.1.8e"') && !configJs.includes('VERSION: "v1.1.8d"') && !configJs.includes('VERSION: "v1.1.8b"')) {
    throw new Error('APP_CONFIG.VERSION must be v1.1.8l in Config.js');
  }
  if (!codeJs.includes('v1.1.8l') && !codeJs.includes('v1.1.8k') && !codeJs.includes('v1.1.8j') && !codeJs.includes('v1.1.8i') && !codeJs.includes('v1.1.8h') && !codeJs.includes('v1.1.8g') && !codeJs.includes('v1.1.8f') && !codeJs.includes('v1.1.8e') && !codeJs.includes('v1.1.8d') && !codeJs.includes('v1.1.8b')) {
    throw new Error('Version must be v1.1.8l in Code.js');
  }
  if (!dbServiceJs.includes('v1.1.8l') && !dbServiceJs.includes('v1.1.8k') && !dbServiceJs.includes('v1.1.8j') && !dbServiceJs.includes('v1.1.8i') && !dbServiceJs.includes('v1.1.8h') && !dbServiceJs.includes('v1.1.8g') && !dbServiceJs.includes('v1.1.8f') && !dbServiceJs.includes('v1.1.8e') && !dbServiceJs.includes('v1.1.8d') && !dbServiceJs.includes('v1.1.8b')) {
    throw new Error('Version must be v1.1.8l in DatabaseService.js');
  }
  ...
  const configV118d = fs.readFileSync('App_Script/Config.js', 'utf8');
  const auditCtrlV118d = fs.readFileSync('App_Script/AuditController.html', 'utf8');

  // 1. Camera Constraints Syntax (RCA: Standard Html5Qrcode { facingMode: currentFacingMode } without nested ideal objects)
  if (!scannerCtrlV118d.includes('primaryConstraint = { facingMode: currentFacingMode }') &&
      !scannerCtrlV118d.includes('primaryConstraint = { facingMode: "environment" }')) {
    throw new Error('ScannerController.html must use standard facingMode string constraint without nested ideal objects');
  }
  if (!scannerCtrlV118d.includes('webkit-playsinline')) {
    throw new Error('ScannerController.html must set webkit-playsinline for iOS WebKit video compatibility');
  }
  console.log('  Verified: Camera feed constraints formatted as standard strings and webkit-playsinline enabled for iOS/Android stability.');

  // 2. Telemetry Logging (camera, snap, toast, sync)
  if (!scannerCtrlV118d.includes('AppState.logActivity("camera"') ||
      !scannerCtrlV118d.includes('AppState.logActivity("snap"') ||
      !modalCtrlV118d.includes('AppState.logActivity("toast"')) {
    throw new Error('Codebase must log camera attempts/errors, snap photos, and toast notifications to AppState.logActivity');
  }
  if (!auditCtrlV118d.includes('entry.type === \'camera\'') ||
      !auditCtrlV118d.includes('entry.type === \'snap\'') ||
      !auditCtrlV118d.includes('entry.type === \'toast\'')) {
    throw new Error('AuditController.html renderActivityLogs must style camera, snap, and toast telemetry events');
  }
  console.log('  Verified: Comprehensive telemetry logging active for camera feed attempts, snap photos, toasts, and audit mutations.');

  // 3. Globalized Dynamic Versioning from Config Sheet
  if (!appStateV118d.includes('getVersion') || !appStateV118d.includes('applyGlobalVersion')) {
    throw new Error('AppState.html must expose getVersion and applyGlobalVersion methods for dynamic versioning');
  }
  ...
  if (!configV118d.includes('VERSION: "v1.1.8l"') && !configV118d.includes('VERSION: "v1.1.8k"') && !configV118d.includes('VERSION: "v1.1.8j"') && !configV118d.includes('VERSION: "v1.1.8i"') && !configV118d.includes('VERSION: "v1.1.8h"') && !configV118d.includes('VERSION: "v1.1.8g"') && !configV118d.includes('VERSION: "v1.1.8f"') && !configV118d.includes('VERSION: "v1.1.8e"') && !configV118d.includes('VERSION: "v1.1.8d"')) {
    throw new Error('Config.js must define APP_CONFIG.VERSION as v1.1.8l or previous');
  }
  ...
  // 1. Version v1.1.8h/i/j/k/l in Config.js
  if (!configV118h.includes('VERSION: "v1.1.8l"') && !configV118h.includes('VERSION: "v1.1.8k"') && !configV118h.includes('VERSION: "v1.1.8j"') && !configV118h.includes('VERSION: "v1.1.8i"') && !configV118h.includes('VERSION: "v1.1.8h"')) {
    throw new Error('Config.js must define APP_CONFIG.VERSION as v1.1.8l, v1.1.8k, v1.1.8j, v1.1.8i or v1.1.8h');
  }
  ...
  console.log('  Verified: Config.js getLocalConfig() auto-syncs stale Google Sheet Config cells.');
  ...
  if ((!appStateV118h.includes('CURRENT_RELEASE = "v1.1.8l"') && !appStateV118h.includes('CURRENT_RELEASE = "v1.1.8k"') && !appStateV118h.includes('CURRENT_RELEASE = "v1.1.8j"') && !appStateV118h.includes('CURRENT_RELEASE = "v1.1.8i"') && !appStateV118h.includes('CURRENT_RELEASE = "v1.1.8h"')) ||
  ...
  // 6. ApiClient.html Cache Key and Mock v1.1.8h/i/j/k/l
  ...
  console.log('  Verified: ApiClient.html cache keys and mock fallbacks aligned.');

  // ==========================================
  // SUITE 44: v1.1.8i Release Audit & Flexible Role RBAC & Bi-Sync Resilience
  // ==========================================
  console.log('\n--- Running Suite 44: v1.1.8i Flexible Role RBAC & Bi-Sync Resilience ---');
  const authServiceV118i = fs.readFileSync('App_Script/AuthService.js', 'utf8');
  const configV118i = fs.readFileSync('App_Script/Config.js', 'utf8');
  const dbServiceV118i = fs.readFileSync('App_Script/DatabaseService.js', 'utf8');
  const auditCtrlV118i = fs.readFileSync('App_Script/AuditController.html', 'utf8');
  const apiClientV118i = fs.readFileSync('App_Script/ApiClient.html', 'utf8');
  ...
  // 1. Version v1.1.8i/j/k/l Alignment
  if (!configV118i.includes('VERSION: "v1.1.8l"') && !configV118i.includes('VERSION: "v1.1.8k"') && !configV118i.includes('VERSION: "v1.1.8j"') && !configV118i.includes('VERSION: "v1.1.8i"')) {
    throw new Error('Config.js must define APP_CONFIG.VERSION as v1.1.8l, v1.1.8k, v1.1.8j or v1.1.8i');
  }
  if (!appStateV118i.includes('CURRENT_RELEASE = "v1.1.8l"') && !appStateV118i.includes('CURRENT_RELEASE = "v1.1.8k"') && !appStateV118i.includes('CURRENT_RELEASE = "v1.1.8j"') && !appStateV118i.includes('CURRENT_RELEASE = "v1.1.8i"')) {
    throw new Error('AppState.html CURRENT_RELEASE must be v1.1.8l, v1.1.8k, v1.1.8j or v1.1.8i');
  }
  if (!apiClientV118i.includes('MUIDS_ASSET_CACHE_v1.1.8l') && !apiClientV118i.includes('MUIDS_ASSET_CACHE_v1.1.8k') && !apiClientV118i.includes('MUIDS_ASSET_CACHE_v1.1.8j') && !apiClientV118i.includes('MUIDS_ASSET_CACHE_v1.1.8i')) {
    throw new Error('ApiClient.html must use MUIDS_ASSET_CACHE_v1.1.8l, v1.1.8k, v1.1.8j or v1.1.8i');
  }
  console.log('  Verified: All modules aligned to semantic release.');

  // 2. AuthService Flexible Role RBAC & SuperAdmin Guarantee
  if (!authServiceV118i.includes('AUTHORIZED_USERS_CACHE_v1.1.8l') && !authServiceV118i.includes('AUTHORIZED_USERS_CACHE_v1.1.8k') && !authServiceV118i.includes('AUTHORIZED_USERS_CACHE_v1.1.8j') && !authServiceV118i.includes('AUTHORIZED_USERS_CACHE_v1.1.8i')) {
    throw new Error('AuthService.js must use cache key AUTHORIZED_USERS_CACHE_v1.1.8l, v1.1.8k, v1.1.8j or v1.1.8i');
  }
  if (!authServiceV118i.includes('normalizedRole.indexOf("admin") !== -1') ||
      !authServiceV118i.includes('normalizedRole.indexOf("tech") !== -1') ||
      !authServiceV118i.includes('normalizedRole.indexOf("ta") !== -1')) {
    throw new Error('AuthService.js must flexibly match admin, tech, ta variations in role strings');
  }
  if (!authServiceV118i.includes('email.indexOf("nattapat.poo") !== -1')) {
    throw new Error('AuthService.js must unconditionally guarantee SuperAdmin for nattapat.poo@mahidol.ac.th');
  }
  console.log('  Verified: AuthService.js provides flexible role matching (Admin/LabTech/TA) and SuperAdmin guarantee.');

  // 3. DatabaseService getInitialPayload & updateAssetStatus Bi-Sync Resilience
  if (!dbServiceV118i.includes('function getInitialPayload(clientEmail)') ||
      !dbServiceV118i.includes('authenticateSession(clientEmail)')) {
    throw new Error('DatabaseService.js getInitialPayload must accept and forward clientEmail to authenticateSession');
  }
  if (!dbServiceV118i.includes('targetSheetName === MASTER_SHEET_NAME') ||
      !dbServiceV118i.includes('setupRow5AuditHeaders(ss, [roomSheet.getName()])')) {
    throw new Error('DatabaseService.js updateAssetStatus must resolve individual room sheet from Master Table and optimize row 5 header setup');
  }
  console.log('  Verified: DatabaseService.js forwards clientEmail and performs high-speed targeted bi-sync.');

  // 4. AuditController Error Handling & Client Email Forwarding
  if (!auditCtrlV118i.includes('res.success === false') ||
      !auditCtrlV118i.includes('บันทึกไม่สำเร็จ')) {
    throw new Error('AuditController.html must verify res.success and show error toast on rejection');
  }
  if (!auditCtrlV118i.includes('clientEmail: currentUser.email')) {
    throw new Error('AuditController.html must attach clientEmail to updateAssetStatus payload');
  }
  console.log('  Verified: AuditController.html protects against silent mutation failures and attaches clientEmail.');

  // 5. ModalController & AppState Resilient Verification Checks
  if (!modalCtrlV118i.includes('isSuperAdmin') ||
  ...
  // 1. Semantic Version Alignment v1.1.8j/k/l
  if (!configV118j.includes('VERSION: "v1.1.8l"') && !configV118j.includes('VERSION: "v1.1.8k"') && !configV118j.includes('VERSION: "v1.1.8j"')) {
    throw new Error('Config.js must define APP_CONFIG.VERSION as v1.1.8l, v1.1.8j or v1.1.8k');
  }
  if (!appStateV118j.includes('CURRENT_RELEASE = "v1.1.8l"') && !appStateV118j.includes('CURRENT_RELEASE = "v1.1.8k"') && !appStateV118j.includes('CURRENT_RELEASE = "v1.1.8j"')) {
    throw new Error('AppState.html CURRENT_RELEASE must be v1.1.8l, v1.1.8j or v1.1.8k');
  }
  if (!apiClientV118j.includes('MUIDS_ASSET_CACHE_v1.1.8l') && !apiClientV118j.includes('MUIDS_ASSET_CACHE_v1.1.8k') && !apiClientV118j.includes('MUIDS_ASSET_CACHE_v1.1.8j')) {
    throw new Error('ApiClient.html must use MUIDS_ASSET_CACHE_v1.1.8l, v1.1.8j or v1.1.8k');
  }
  ...
  if (!auditCtrlV118j.includes('watchdogTimer') || !auditCtrlV118j.includes('45000')) {
  ...
  // ==========================================
  // SUITE 46: v1.1.8k Live Save Pipeline & Sync Audit Hardening
  // ==========================================
  console.log('\n--- Running Suite 46: v1.1.8k Live Save Pipeline & Sync Audit Hardening ---');
  const auditCtrlV118k = fs.readFileSync('App_Script/AuditController.html', 'utf8');
  const apiClientV118k = fs.readFileSync('App_Script/ApiClient.html', 'utf8');
  const dbServiceV118k = fs.readFileSync('App_Script/DatabaseService.js', 'utf8');
  const configV118k = fs.readFileSync('App_Script/Config.js', 'utf8');
  const appStateV118k = fs.readFileSync('App_Script/AppState.html', 'utf8');
  const authServiceV118k = fs.readFileSync('App_Script/AuthService.js', 'utf8');

  // 1. Semantic Version Alignment v1.1.8k/l
  if (!configV118k.includes('VERSION: "v1.1.8l"') && !configV118k.includes('VERSION: "v1.1.8k"')) {
    throw new Error('Config.js must define APP_CONFIG.VERSION as v1.1.8k or v1.1.8l');
  }
  if (!appStateV118k.includes('CURRENT_RELEASE = "v1.1.8l"') && !appStateV118k.includes('CURRENT_RELEASE = "v1.1.8k"')) {
    throw new Error('AppState.html CURRENT_RELEASE must be v1.1.8k or v1.1.8l');
  }
  if (!apiClientV118k.includes('MUIDS_ASSET_CACHE_v1.1.8l') && !apiClientV118k.includes('MUIDS_ASSET_CACHE_v1.1.8k')) {
    throw new Error('ApiClient.html must use MUIDS_ASSET_CACHE_v1.1.8k or v1.1.8l');
  }
  if (!authServiceV118k.includes('AUTHORIZED_USERS_CACHE_v1.1.8l') && !authServiceV118k.includes('AUTHORIZED_USERS_CACHE_v1.1.8k')) {
    throw new Error('AuthService.js must use AUTHORIZED_USERS_CACHE_v1.1.8k or v1.1.8l');
  }
  console.log('  Verified: All core modules cleanly bumped to v1.1.8k.');

  // 2. ApiClient Live Mode Everywhere & Anti-Ghost Mutation Protection
  if (!apiClientV118k.includes('window.location.search.indexOf("mock=1") === -1') ||
      apiClientV118k.includes('window.location.hostname !== "localhost"')) {
    throw new Error('ApiClient.html isRemoteLiveMode must be enabled across all hosts unless ?mock=1 is present');
  }
  if (!apiClientV118k.includes('Mutations MUST NEVER silently mock-succeed') ||
      !apiClientV118k.includes('action === "updateAssetStatus" || action === "addNewUnlistedAsset"')) {
    throw new Error('ApiClient.html must prevent mutations from mock-succeeding when network bridges fail');
  }
  console.log('  Verified: Live mode active by default across all hosts; ghost mock mutations prohibited.');

  // 3. Status String Normalization (+ vs space) in DatabaseService.js
  if (!dbServiceV118k.includes('opt.replace(/[\\s\\+]/g, \'') ||
      !dbServiceV118k.includes('newStatus.replace(/[\\s\\+]/g, \'')) {
    throw new Error('DatabaseService.js updateAssetStatus must normalize status strings removing spaces and plus signs');
  }
  if (!dbServiceV118k.includes('getRoomAuditSummary(currentRoom || targetSheetName || "ALL")')) {
    throw new Error('DatabaseService.js must respect currentRoom when returning roomSummary in updateAssetStatus');
  }
  console.log('  Verified: DatabaseService.js tolerates URL-decoded spaces in place of + and respects currentRoom.');

  // 4. AuditController Toast Sequencing & Room Summary Integrity
  if (!auditCtrlV118k.includes('กำลังบันทึก') ||
      !auditCtrlV118k.includes('บันทึกสำเร็จ:') ||
      !auditCtrlV118k.includes('res.roomSummary.roomName === activeRoom')) {
    throw new Error('AuditController.html must show in-flight toast, confirmed success toast, and guard roomSummary integrity');
  }
  console.log('  Verified: AuditController.html provides accurate in-flight feedback and preserves room checklist integrity.');

  // 5. AuthService Variable Safety
  if (!authServiceV118k.includes('var role = "Auditor";') ||
      !authServiceV118k.includes('var isTALT = false;')) {
    throw new Error('AuthService.js must safely declare role and isTALT in fallback branch');
  }
  console.log('  Verified: AuthService.js fallback resolution declares all local variables safely.');

  // ==========================================
  // SUITE 47: v1.1.8l Non-Truncated Toasts/Logs & Full Save Telemetry
  // ==========================================
  console.log('\n--- Running Suite 47: v1.1.8l Non-Truncated Toasts/Logs & Full Save Telemetry ---');
  const auditCtrlV118l = fs.readFileSync('App_Script/AuditController.html', 'utf8');
  const modalCtrlV118l = fs.readFileSync('App_Script/ModalController.html', 'utf8');
  const apiClientV118l = fs.readFileSync('App_Script/ApiClient.html', 'utf8');
  const configV118l = fs.readFileSync('App_Script/Config.js', 'utf8');
  const appStateV118l = fs.readFileSync('App_Script/AppState.html', 'utf8');
  const authServiceV118l = fs.readFileSync('App_Script/AuthService.js', 'utf8');

  // 1. Semantic Version Alignment v1.1.8l
  if (!configV118l.includes('VERSION: "v1.1.8l"')) {
    throw new Error('Config.js must define APP_CONFIG.VERSION as v1.1.8l');
  }
  if (!appStateV118l.includes('CURRENT_RELEASE = "v1.1.8l"')) {
    throw new Error('AppState.html CURRENT_RELEASE must be v1.1.8l');
  }
  if (!apiClientV118l.includes('MUIDS_ASSET_CACHE_v1.1.8l')) {
    throw new Error('ApiClient.html must use MUIDS_ASSET_CACHE_v1.1.8l');
  }
  if (!authServiceV118l.includes('AUTHORIZED_USERS_CACHE_v1.1.8l')) {
    throw new Error('AuthService.js must use AUTHORIZED_USERS_CACHE_v1.1.8l');
  }
  console.log('  Verified: All core modules cleanly bumped to v1.1.8l.');

  // 2. Non-Truncated Toast Elements in ModalController.html
  if (modalCtrlV118l.includes('textSpan.className = "toast-message-text truncate') ||
      !modalCtrlV118l.includes('toast-message-text break-words whitespace-normal') ||
      !modalCtrlV118l.includes('rounded-2xl') ||
      !modalCtrlV118l.includes('max-w-sm sm:max-w-md w-auto')) {
    throw new Error('ModalController.html showToast must not truncate text and must use break-words whitespace-normal');
  }
  console.log('  Verified: ModalController.html toast notifications render full text without CSS truncate.');

  // 3. Non-Truncated Activity Logs in AuditController.html
  if (auditCtrlV118l.includes('text-[11px] text-slate-400 truncate') ||
      auditCtrlV118l.includes('text-[11px] text-slate-300 truncate') ||
      !auditCtrlV118l.includes('break-words whitespace-normal leading-snug')) {
    throw new Error('AuditController.html renderActivityLogs must not truncate item names and must use break-words whitespace-normal');
  }
  console.log('  Verified: AuditController.html activity logs display complete asset item names and notes.');

  // 4. Extended Watchdog Timer (45s) & Full Audit Lifecycle Logging
  if (!auditCtrlV118l.includes('watchdogTimer = setTimeout') ||
      !auditCtrlV118l.includes('45000') ||
      !auditCtrlV118l.includes('⏳ กำลังส่งบันทึก:') ||
      !auditCtrlV118l.includes('✅ บันทึกสำเร็จ:') ||
      !auditCtrlV118l.includes('❌ บันทึกไม่สำเร็จ:')) {
    throw new Error('AuditController.html must have 45s watchdog and log in-flight, success, and error lifecycle events to AppState');
  }
  console.log('  Verified: AuditController.html extended watchdog (45s) and full audit lifecycle telemetry active.');

  // 5. In-Place Master Table Summary Sync
  if (!auditCtrlV118l.includes('activeRoom === "Master_Asset"') ||
      !auditCtrlV118l.includes('curSummary.items[foundIdx]') ||
      !auditCtrlV118l.includes('percentComplete = Math.round')) {
    throw new Error('AuditController.html must mutate AppState.summary in-place for Master_Asset updates');
  }
  console.log('  Verified: In-place Master Table checklist summary mutation enabled.');

  console.log('\n======================================================');
  console.log('✅ ALL 47 TEST SUITES PASSED FOR v1.1.8l RELEASE AUDIT VERIFICATION!');
  console.log('======================================================\n');
}
